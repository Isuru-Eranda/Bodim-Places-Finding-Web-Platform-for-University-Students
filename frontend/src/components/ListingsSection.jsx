...
export default function ListingsSection({ listings, loading, error }) {
  const displayListings = listings && listings.length > 0 ? listings.slice(0, 4) : MOCK_LISTINGS;
    ...
    <section className="bg-[#FFFDFB] py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl lg:text-3xl font-bold text-[#1F2937]">Popular Listings</h2>
        ...
        {!loading && !error && listings && listings.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-semibold text-[#1F2937] mb-2">No listings found</h3>
            <p className="text-[#6B7280] text-sm">Try adjusting your search filters to find more options.</p>
          </div>
        )}
      </div>
    </section>
